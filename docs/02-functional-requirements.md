...
- **目的**: §4.1 工程 8、Phase 3。
- **入力**: `books.id`、`kdp_metadata`、成果物 URL（docx/pdf/png）
- **処理**: Playwright で KDP にログイン → メタデータ入力 → 本文/カバーアップロード → 価格設定 → 「公開」を保留。2FA 必要時は運営者にプッシュ通知し承認入力を待つ
- **出力**: 入稿ジョブ結果、ASIN（取得できれば）
- **受け入れ基準**:
  - 1 冊あたり 10 分以内に「公開待ち」状態まで到達
  - 2FA は最長 10 分待機後タイムアウト
  - エラー時はスクリーンショットを保存
- **関連エージェント**: N/A
