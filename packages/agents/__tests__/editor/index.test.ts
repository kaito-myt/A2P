...
describe('editBook — 章数 / index 検証', () => {
  it('入力 8 章 / 出力 7 章 → AgentError(chapters_mismatch)', async () => {
    const input = baseInput();
    // 7 章のみ返す (最後の 1 章を欠落)
    const chaptersOverride = input.chapters.slice(0, 7).map((c, i) => {
      const isLast = i === 6; // 出力上の最終章
      return {
        index: c.index,
        heading: c.heading,
        body_md: isLast
    ...
    expect((caught as AgentError).message).toMatch(/chapters_mismatch/);
    const details = (caught as AgentError).details as {
      input_count: number; output_count: number;
    };
    expect(details.input_count).toBe(8);
    expect(details.output_count).toBe(7);
  });

  it('index 不連続 (1,2,4,...) → AgentError(chapters_mismatch: index)', async () => {
    const input = baseInput({
      chapters: Array.from({ length: 8 }, (_, i) => sampleInputChapter(i + 1)),
    });
    // 出力は同じ章数 (8) だが、3 番目の index を 4 にする (入力は 3)
    const chaptersOverride = input.chapters.map((c, i) => {
      const isLast = i === 7;
      const idx = i === 2 ? 4 : c.index;
      return {
        index: idx,
        heading: c.heading,
        body_md: isLast
    ...
    expect(caught).toBeInstanceOf(AgentError);
    expect((caught as AgentError).message).toMatch(/chapters_mismatch/);
    expect((caught as AgentError).message).toMatch(/index/);
  });
});
...
describe('editBook — プロンプト差込', () => {
  ...
  it('プレースホルダ ({theme_title}/{ai_disclosure_text}/{chapter_count}/{draft_chapters}) が system に差し込まれる', async () => {
    ...
    expect(systemMsg!.content).toContain('カスタム開示文 (テスト用)');
    expect(systemMsg!.content).toContain('8'); // chapter_count
